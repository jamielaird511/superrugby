...
const RESULTS_FETCH_INIT: RequestInit = {
  cache: "no-store",
  next: { revalidate: 0 },
  headers: {
...
function resultSummary(
  ...
  const scoreLine = `${left} ${homeGoals}–${awayGoals} ${right}`;
  if (!penaltyWinnerCode) return scoreLine;
  const penLabel = teamLabel(penaltyWinnerCode, teamNames);
  return `${scoreLine} (${penLabel} pens)`;
}
...
export default function WorldCupResultsClient() {
    ...
    async (options?: { background?: boolean }) => {
      ...
      try {
        const res = await fetch(
          `/api/worldcup/results?tenant=${tenant.slug}`,
          RESULTS_FETCH_INIT
        );
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      void fetchResults({ background: true });
    }, 5000);
